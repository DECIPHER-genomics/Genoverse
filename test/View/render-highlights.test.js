...
describe('Correctly render highlights:', () => {
  ...
  const getInstructions = draw => ({
    forward: {
      features   : [[ 0, 0, 1000, 15, '#FFFFFF' ]].concat(draw.features),
      labels     : draw.labels,
      background : draw.background,
    },
    reverse: { // Remove feature offset, no white background
      features: draw.features.map((f) => {
        f    = [].concat(f);
        f[1] = 0;
  ...
  describe('backgrounds', () => {
    const drawBorder = (context, x1, x2, color) => {
    ...
    it('overlapping highlights, completely inside region', () => {
      const draw = {
        features   : [[ 99, 13, 101, 2, 'red' ], [ 109, 13, 11, 2, 'blue' ]],
        background : [
          [ 99,  0, 101, 1, '#FFCCCC' ], (context) => { drawBorder(context, 99.5,  200.5, 'red');  },
          [ 109, 0, 11,  1, '#CCCCFF' ], (context) => { drawBorder(context, 109.5, 120.5, 'blue'); },
        ],
      };
    ...
    it('only left highlight edge inside region', () => {
      const draw = {
        features   : [[ 0, 13, 500, 2, 'red' ]],
        background : [
          [ 0, 0, 500, 1, '#FFCCCC' ], (context) => { drawBorder(context, false, 500.5, 'red'); },
        ],
      };
    ...
    it('only right highlight edge inside region', () => {
      const draw = {
        features   : [[ 99, 13, 901, 2, 'red' ]],
        background : [
          [ 99, 0, 901, 1, '#FFCCCC' ], (context) => { drawBorder(context, 99.5, false, 'red'); },
        ],
      };
    ...
    it('neither highlight edge inside region', () => {
      const draw = {
